...
export class FreshMigration1751345052344 implements MigrationInterface {
    name = 'FreshMigration1751345052344'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "doctor_time_slots" ("timeslot_id" SERIAL NOT NULL, "date" date NOT NULL, "startTime" TIME NOT NULL, "endTime" TIME NOT NULL, "status" "public"."doctor_time_slots_status_enum" NOT NULL DEFAULT 'available', "maxPatients" integer, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "doctor_id" integer, "availability_id" integer, CONSTRAINT "PK_8ae12dfe3687c8c338b5f24f4cf" PRIMARY KEY ("timeslot_id"))`);
        await queryRunner.query(`CREATE TABLE "doctor_availabilities" ("availability_id" SERIAL NOT NULL, "date" date NOT NULL, "startTime" TIME NOT NULL, "endTime" TIME NOT NULL, "session" "public"."doctor_availabilities_session_enum" NOT NULL, "weekdays" "public"."doctor_availabilities_weekdays_enum" array, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "doctor_id" integer, CONSTRAINT "UQ_34ae84a982de88612ac9fd7dca6" UNIQUE ("doctor_id", "date", "startTime", "endTime"), CONSTRAINT "PK_b72e631ea2d3dc4d641e030ccc1" PRIMARY KEY ("availability_id"))`);
        await queryRunner.query(`CREATE TYPE "public"."doctors_schedule_type_enum" AS ENUM('stream', 'wave')`);
        await queryRunner.query(`CREATE TABLE "doctors" ("user_id" integer NOT NULL, "education" character varying NOT NULL, "specialization" character varying NOT NULL, "experience_years" integer NOT NULL, "clinic_name" character varying NOT NULL, "clinic_address" character varying NOT NULL, "schedule_type" "public"."doctors_schedule_type_enum" NOT NULL DEFAULT 'stream', "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_653c27d1b10652eb0c7bbbc4427" PRIMARY KEY ("user_id"))`);
